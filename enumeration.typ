...
+ Every normalizable $lambda$-term has a unique normal form, so we never generate two $beta$-equivalent terms. #footnote[However, we may still generate terms which are equivalent in a particular language (for example $(lambda x y . (+) x y)$ is equivalent to $(lambda x y . (+) y x )$), or which are $eta$-equivalent. Both of these concerns can be addressed using semantic analysis.]
...
Whenever we begin a new search, we consult the cache to see if this query has been made before. If so, we either skip the search (if it's `Empty`), or use the cached values (before picking up the search at the point where the previous search ran out of space in the cache). This is extremely useful, since many search queries yield few (if any) results, even if the search would be very dificult.

=== Argument Pruning

#let app_link = link(<App>)[(App)]
